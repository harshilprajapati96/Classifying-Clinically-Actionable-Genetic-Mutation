...
function svm_group = training_SA_SVM(X_train,Y_train,Y_train_expand,vocab_len,tuning,mode)
...
[X_train_1_2, Y_train_1_2] ...
    = ovo(pair_i(i),pair_j(i),X_train,Y_train,Y_train_expand,vocab_len);
...
parfor i = 1:m
[X_train_1_2, Y_train_1_2] ...
    = ova(pair_i(i),X_train,Y_train,Y_train_expand,vocab_len);
kfold = cvpartition(length(Y_train_1_2),'KFold',k); 
svm_ccr = zeros(k,length(tuning));
    for j = 1:length(tuning)
       for i_fold = 1:k
